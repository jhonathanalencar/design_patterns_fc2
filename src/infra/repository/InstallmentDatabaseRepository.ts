...
export class InstallmentDatabaseRepository implements InstallmentRepository {
  constructor(readonly connection: Connection) {}

  async save(installment: Installment): Promise<void> {
    await this.connection.query(
      "INSERT INTO cifer.installment (loan_code, number, amount, interest, amortization, balance) VALUES ($1, $2, $3, $4, $5, $6)",
      [
  ...
  async getByCode(code: string): Promise<Installment[]> {
    const installmentsData = await this.connection.query(
      "SELECT * FROM cifer.installment WHERE loan_code = $1",
      [code]
    );
